...
var Login = {
  ...
  Login_Check_SKP: function(User_Name_, Password_, callback) {
    return db.query(
      "CALL Login_Check_SKP (@User_Name_ :=?,@Password_ :=?)",
      [User_Name_, Password_],
      callback
    );
  },
  ...
  Candidate_Login: function(Email_, Password_, callback) {
    return db.query("CALL Candidate_Login (@Email_ :=?,@Password_ :=?)",
      [Email_, Password_],
      callback
    );
  },
