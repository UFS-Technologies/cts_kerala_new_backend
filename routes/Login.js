...
const config = { secret: process.env.JWT_SECRET || require('../config.json').secret };
...
router.get("/Login_Check_SKP/:userName?/:password?", function(req, res, next) {
  try {
    Login.Login_Check_SKP(req.params.userName, req.params.password, function(
      err,
      rows
...
router.get("/Candidate_Login/:Email?/:Password?", function(req, res, next) {
  try {
    Login.Candidate_Login(req.params.Email, req.params.Password, function(err,rows) 
    {
      if (err) {
        res.json(err);
      }
